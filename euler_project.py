...
@time_execution
def odd_sum(x: int = 5) -> int:
    '''
    Problem 0:
    Compute the sum of the squares of all odd natural numbers from 1 up to and including x.

    For each integer i in the range [1, x], only those i for which both i and i² are odd
    (i.e. i is odd) are considered, and i² is added to the running total.

    arguments:
    x -- upper bound of the range of natural numbers to consider (default 712)

    returns:
    The sum of i² for all odd integers i such that 1 ≤ i ≤ x.
    '''
    odd_sum = 0
...
def even_fibonacci_numbers(x: int = 4000000) -> int:
    ...
    while sum(fibonacci_seq[-2:]) < x:
        fibonacci_seq.append(sum(fibonacci_seq[-2:]))
        if sum(fibonacci_seq[-2:]) % 2 == 0:
            even_fibonacci_sum += sum(fibonacci_seq[-2:])
...
@time_execution
def highly_divisible_triangular_number(x: int = 500):
    '''
    Problem 12:
...
def large_sum() -> int:
    ...
    summa = sum([int(digit[d:d+50]) for d in range(0, len(digit), 50)])
    return str(summa)[:10]
...
def longest_collatz_sequence(n: int):
    ...
    '''
    pass
    def generate_collatz_seq(n: int):
        if n < 1:
            return [n]
        collatz_seq = [n]
        while n != 1:
    ...
    longest_chain = {'number': n, 'chain_len': 0}
    for x in range(n, 0, -1):
        chain_len = len(generate_collatz_seq(x))
        if longest_chain['chain_len'] < chain_len:
            longest_chain['number'] = x
            longest_chain['chain_len'] = chain_len
    return longest_chain

@time_execution
def lattice_paths(demension: int = 20):
    '''
    Problem 15:
    ...
    '''
    if demension > 10:
        return int(math.factorial(2 * demension) / (math.factorial(demension)**2))
        # otherwise, calculate paths

    def get_forks(coords):
        forks = []
        if coords[0] + 1 <= demension:
            forks.append((coords[0] + 1, coords[1]))
        if coords[1] + 1 <= demension:
            forks.append((coords[0], coords[1] + 1))
        return forks

    start = (0, 0)
    generation = get_forks(start)
    paths = [[start, fork] for fork in generation]
    while len(generation) != 0:
        print(generation)
        next_generation = []
        for fork in generation:
            childs = get_forks(fork)
            ...
            for path in paths:
                if path[-1] == fork:
                    for child in childs:
                        cpath = path[:]
                        cpath.append(child)
                        path_upgrade.append(cpath)
                else:
                    path_upgrade.append(path)
            paths = path_upgrade
...
@time_execution
def power_digit_sum(power: int = 1000):
    '''
    Problem 16:
    2^15 = 32768 and the sum of its digits is 3 + 2 + 7 + 6 + 8 = 26.
    What is the sum of the digits of the number 2^1000?
    '''
    return sum([ int(val) for val in str(2 ** power) ])

@time_execution
def number_letter_counts(end: int = 30):
    '''
    Problem 17:
    ...
    - $115$ (one hundred and fifteen) contains $20$ letters. The use of "and" when writing out numbers is in compliance with British usage.</p>
    '''
    pass
    digit_spelling_map = {
        1: 'one',
    ...
    def split_into_multipliers(digit: int):
        value = digit
        power_10 = len(str(digit)) - 1
        out = ''
        for i in range(power_10, -1, -1):
            out += f'{digit // 10**i} * {10**i} + '
            digit -= (digit // 10**i) * 10**i
        print(f'{value}: {out}')
...
if __name__ == '__main__':
    print('Problem 0:', odd_sum(216000))
    ...
    print('Problem 13: ', large_sum())
    # print('Problem 14: ', longest_collatz_sequence(999999))
    print('Problem 15: ', lattice_paths())
    print('Problem 16: ', power_digit_sum())
    # print('Problem 17: ', number_letter_counts())